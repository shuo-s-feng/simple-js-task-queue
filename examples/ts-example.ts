import { TaskQueue, TaskId, Task, TaskStatus } from "simple-js-task-queue";
...
const generateTask = (id: TaskId, wait: number) => {
  return async () => {
...
const handleTaskResult = (taskId: TaskId, result: any) => {
  console.log(
    `After ${
...
const handleTaskStatusChange = (status: TaskStatus, task: Task) => {
  console.log(
    `After ${
      (new Date().getTime() - start) / 1000
